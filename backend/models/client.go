...
type Client struct {
	...
	// 注意：使用普通索引而非唯一索引 —— 旧版客户端/采集失败时会写入空串，
	// MySQL 唯一索引不豁免重复空串，会导致第二台旧版客户端注册永久失败；
	// 去重由注册流程的查询逻辑保证（见 Register 优先级 1）
	HardwareUUID   string         `gorm:"column:hardware_uuid;size:64;index:idx_hardware_uuid" json:"hardware_uuid"` // 硬件唯一标识 (Windows System UUID)
