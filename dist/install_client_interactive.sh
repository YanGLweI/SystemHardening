...
if [ -n "$1" ]; then
    ...
    echo "-------------------------------"
    read -p "Enter backend server URL (e.g., http://10.60.254.191:8080): " SERVER_URL
...
echo "   c. Save tokens to SQLite database:"
echo "      sqlite3 ${DATA_DIR}/tokens.db \"CREATE TABLE IF NOT EXISTS tokens(id INTEGER PRIMARY KEY,short_token TEXT NOT NULL,refresh_token TEXT NOT NULL,expires_at TEXT NOT NULL);\""
echo "      sqlite3 ${DATA_DIR}/tokens.db \"INSERT OR REPLACE INTO tokens VALUES(1,'YOUR_SHORT_TOKEN','YOUR_REFRESH_TOKEN','2026-08-15T00:00:00Z');\""
echo ""
